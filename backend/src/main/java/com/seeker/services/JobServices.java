...
public class JobServices {
	...
	public Object getJobsAtMyCity(AddressDTO addressDto) {
		...
		return jobRepo.findAll().stream()
				...
				.map(e -> mapper.map(e, JobDTO.class)).collect(Collectors.toList());
	}
...
//	Create Job
	...
	public Object getJob(String id) {
		...
		return mapper.map(jobRepo.findById(Long.parseLong(id)).orElseThrow(() -> new BackendException("Job not Found")),
				JobDTO.class);
	}
